...
pub async fn get_user_by_id(
    ...
    Ok(web::block(move || db::users::db_get_user_by_id(db.into_inner(), user_id.into_inner()))
        .await
        .map(|user| HttpResponse::Ok().json(user))
        .map_err(|_| HttpResponse::InternalServerError())?)
}
...
pub async fn add_user(
    ...
    Ok(web::block(move || db::users::add_single_user(db.into_inner(), item.into_inner()))
        .await
