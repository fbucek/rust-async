...
mod database {
    ...
    async fn test_complex() {
        ...
        let password = "strong_pass".to_string();
        let email = "john.doe@apple.com".to_string();
        let user = InputUser {
    ...
    async fn test_auth() {
        ...
        assert!(!db::users::signup_user(pool.clone(), &user).is_ok());

        // Login non existing user
        let login_request: LoginRequest =
            serde_json::from_str(r#"{"username": "nonexisting", "password": "fake"}"#).unwrap();
        assert!(!db::users::login_user(pool.clone(), &login_request).is_ok());
        ...
        let invalid_user = "johnDoe".to_string();
        assert!(!db::users::is_valid_login_session(
            pool.clone(),
            &valid_user,
            &invalid_uuid
        ...
        // Loging out with invalid user must return error
        assert!(!db::users::logout_user(pool.clone(), &invalid_user).is_ok());

        // Login sesssion for valid user must still be valid
        assert!(db::users::is_valid_login_session(
            pool.clone(),
            &valid_user,
            &valid_uuid
        ...
        // After succesfull loging out login session must be invalid
        assert!(db::users::logout_user(pool.clone(), &valid_user).is_ok());
        assert!(!db::users::is_valid_login_session(
            pool.clone(),
            &valid_user,
            &valid_uuid
